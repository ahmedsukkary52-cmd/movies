...
import 'package:moives/features/movies/domain/usecases/movie_suggestions_use_case.dart';
...
class DetailsCubit extends Cubit<DetailsStates> {
  DetailsCubit({required this.useCase, required this.suggestionsUseCase})
      : super(DetailsLoading());
  MovieDetailsUseCase useCase;
  MovieSuggestionsUseCase suggestionsUseCase;
  ...
  Future<void> getMovieSuggestions({required int movieId}) async {
    final response = await suggestionsUseCase.call(movieId);
    response.fold(
          (failure) => null,
          (suggestions) =>
          emit(DetailsSuccess(
            movieDetailsData: state is DetailsSuccess
                ? (state as DetailsSuccess).movieDetailsData
                : null,
            screenShots: state is DetailsSuccess ? (state as DetailsSuccess)
                .screenShots : [],
